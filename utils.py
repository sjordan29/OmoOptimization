...
def PlotIt(table, color, linestyle, label, ax,d,s,u):
    xs = range(len(table.iloc[:, 0]))
    for col in table.columns:
        ax.plot(xs, table[col], c=color, linewidth = 2, label=label if (d==1) or (s==1) or (u==1) else "",
                linestyle=linestyle, zorder = 2 if label == "RC" else 1)
        d+=1 
        s+=1
        u+=1
...
def PAP_ltd(ax, dps_dict, swat_dict, uc_dict, obj,dps_cols, swat_cols, dps_addl, swat_addl, uc_addl):
    dps_table = dps_dict[obj]
    swat_table = swat_dict[obj]
    uc_table = uc_dict[obj]
    ...
    s=2
    u=2
    PlotIt(swat_scaled, 'lightgrey', 'solid', None, ax,d,s,u)
    PlotIt(dps_scaled, 'lightgrey', 'solid', None, ax,d,s,u)
    s=1
    PlotIt(swat_scaled[swat_cols], '#377eb8', 'solid', 'Storage Target', ax, d,s,u) 
    d=1
    PlotIt(dps_scaled[dps_cols], '#ff7f00', 'solid', 'Release Policy', ax, d, s, u)
    u=1
    PlotIt(uc_scaled, 'black', 'dashed', 'Uncontrolled', ax, d, s, u)
