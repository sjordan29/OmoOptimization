...
def envRecPlots(TxtFold_dps, TxtFold_swat, dates, title):
    ...
    for obj,c,fold in zip(Objectives,colors, folders):
        ...
        ax1.plot(swat_mean_rec.index, swat_mean_rec.flow, label=obj,color=c)        
        ax2.plot(dps_mean_rec.index, dps_mean_rec.flow, label=obj, color=c)
        ax3.plot(swat_mean_env.index, swat_mean_env.flow_x, label=obj,color=c)
        ...
        ax1.set_title("Storage Target: \n Flood Recession", size=12)
        ax2.set_title("Release Policy: \n Flood Recession", size=12)

        ax3.set_title("Storage Target: \n Environmental Flows", size=12)
        ax4.set_title("Release Policy: \n Environmental Flows", size = 12)
